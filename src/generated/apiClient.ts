...
export const getGetEntryResponseMock = (overrideResponse: Partial<Extract<EntryContentResponse, object>> = {}): EntryContentResponse => ({id: faker.string.alpha({length: {min: 10, max: 20}}), path_id: faker.string.alpha({length: {min: 10, max: 20}}), day: faker.date.past().toISOString().slice(0, 10), edit_id: faker.string.alpha({length: {min: 10, max: 20}}), content: faker.string.alpha({length: {min: 10, max: 20}}), image_filenames: Array.from({ length: faker.number.int({min: 1, max: 10}) }, (_, i) => i + 1).map(() => (faker.string.alpha({length: {min: 10, max: 20}}))), ...overrideResponse})
